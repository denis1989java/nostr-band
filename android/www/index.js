$(function () {
  ...
  async function addOnNostr(handler) {
    if (nostr_enabled) await handler();
    else on_nostr_handlers.push(handler);
  }
  ...
  function enableNostr() {
    return new Promise(function (ok) {
      ...
      async function checkNostr() {
        if (has_nostr) {
          nostr_enabled = true;
          for (const h of on_nostr_handlers) await h();
  ...
  function getProfilePicture(p) {
    if (p && p.picture && !p.picture.includes(" ")) return p.picture;
    else return "";
  }
...
`;
  ...
  function formatContent(e, max_size) {
    ...
    if (c.length > max_size) c = c.substring(0, max_size) + "...";
    ...
    for (const i in e.links) {
      ...
      try {
        ...
        for (const match of matches) {
          ...
          if (!intersects(m)) inlines.push(m);
        }
      } catch (e) {}
    }
    ...
    for (const m of inlines) {
      // duplicate tags etc
      if (m.o == last_inline_offset) continue;
      ...
      if (!link.label && link.type == "event") {
        ...
        label =
          note.substring(0, 10) +
          "..." +
          note.substring(note.length - 4, note.length);
      }
      if (link.type == "pubkey") {
        if (!link.label) {
          ...
          label =
            npub.substring(0, 10) +
            "..." +
            npub.substring(npub.length - 4, npub.length);
        }
        label = "@" + label;
      }
...
`;
    ...
    // tail
    if (last_offset < c.length) content += c.substring(last_offset, c.length);
  ...
  function formatEvent(req) {
    ...
    const tm = new Date(
      (u.published_at ? u.published_at : u.created_at) * 1000
    ).toLocaleString();
...
`;
    ...
    let relays = "";
    for (const r of u.relays) relays += (relays ? "," : "") + r;
...
`;
    ...
    const title = u.title
      ? `
<a class="text-muted nostr-event-link" href='${post_href}'><h4 class='mt-2' ${style}>${san(
          u.title
        )}</h4></a>
`
      : "";
...
<div class='row nostr-serp-url' 
 ...
 ${style}
 >
<div class='col ${main ? "main" : ""}'><div class='card mb-3 no-border ${
      offset ? "ms-5" : ""
    }'>
...
<img style='width: ${psize}px; height: ${psize}px' 
 ...
 class="profile ${
   img ? "" : "d-none"
 }" onerror="javascript:replaceImgSrc(this)"></a> 
<a class="nostr-profile-link" href='${profile_href}'>${san(
      author
    )}</a> ${profile_btns} 
</div>
...
`;
      if (u.zap_amount)
        html += `
<span class='nostr-zaps me-2'><i class="bi bi-lightning"></i> ${formatZapAmount(
          u.zap_amount
        )}</span>
`;
      if (u.replies)
...
`;
      if (u.zap_amount)
        html += `
<a href='/${note}/zaps' class="inline-link open-zaps-for me-3"><nobr><b>${formatZapAmount(
          u.zap_amount
        )}</b> sats</nobr></a>
`;
...
`;
  ...
  function formatPerson(req) {
    ...
    const last_tm = new Date(
      (edits ? p.last_tm : p.last_event_tm) * 1000
    ).toLocaleString();
    ...
    if (p.nip05_verified) {
      ...
      nip05 =
        `<i class="bi bi-check-circle" style='padding: 0 2px 0 2px'></i>` +
        domain;
      if (name && name != "_") {
        nip05 = name + nip05;
        nip05_url =
        ...
        if (name != p.name && show_profile) handle = p.name;
      } else {
    ...
    const thumb = formatThumbUrl(
      p.pubkey,
      "picture",
      /* big */ show_profile || req.trending
    );
...
`;
    ...
    const cl_naddr = tools.nip19.naddrEncode({
      pubkey: p.pubkey,
      kind: 3,
      relays: [relay],
      identifier: "",
...
`
    ...
    let relays = "";
    for (const r of p.relays) relays += (relays ? "," : "") + r;
...
`;
    ...
    function formatLN(v, u) {
      ...
      if (lnurl && u && u.startsWith("https://")) {
        ...
        if (dn.length == 2) u = dn[1] + "@" + dn[0].substring(8);
      }
...
`;
      ...
      ln += `
<small class='text-muted ln-address'>🗲 ${vs} ${btn} ${
        u ? "(" + u + ")" : ""
      }</small><br>
`;
    }
...
<a class='${show_profile ? "" : "nostr-profile-link"}' href='${
      show_profile ? img : profile_href
    }'
...
<div class='col'><a class="nostr-profile-link me-1"  href='${profile_href}'>${san(
      name
    )}</a>
...
<p class="card-text mt-1 mb-1 ${show_profile ? "" : "open-profile-text"}">${san(
      p.about
    )}</p>
`;
...
`;
      ...
      html += `
<div>
<span class="${show_profile ? "following" : "open-profile-text"} me-2"><b>${
        p.following_count
      }</b> Following</span>
<span class="${show_profile ? "followed" : "open-profile-text"} me-2"><b>${
        p.followed_count
      }</b> Followers${
        new_followers_count
          ? " <sup><b>+" + new_followers_count + "</b></sup>"
          : ""
      }</span>
`;
      if (show_profile) {
        ...
        if (p.zap_amount)
          html += `
<a href='/${npub}/zaps-received' class="inline-link open-zaps-to me-2"><nobr><b>${formatZapAmount(
            p.zap_amount
          )}</b> sats received</nobr></a>
`;
        if (p.zap_amount_sent)
          html += `
<a href='/${npub}/zaps-sent' class="inline-link open-zaps-by me-2"><nobr><b>${formatZapAmount(
            p.zap_amount_sent
          )}</b> sats sent</nobr></a>
`;
        if (p.zap_amount_processed)
          html += `
<a href='/${npub}/zaps-processed' class="inline-link open-zaps-via me-2"><nobr><b>${formatZapAmount(
            p.zap_amount_processed
          )}</b> sats processed</nobr></a>
`;
      }
      html += `
</div>
...
`;
  ...
  function formatZap(z, type) {
    ...
    const zapper_thumb = zapper_img
      ? formatThumbUrl(zapper_pubkey, "picture")
      : "";
    const zapper_psize = 48;
    const zapper_comment = z.desc.content
      ? `Comment "<em>${san(z.desc.content)}</em>"`
      : "";
    ...
    const provider_img = provider_profile
      ? getProfilePicture(provider_profile)
      : "";
    const provider_thumb = provider_img
      ? formatThumbUrl(provider_pubkey, "picture")
      : "";
    ...
    const target_thumb = target_img
      ? formatThumbUrl(target_pubkey, "picture")
      : "";
...
<img style='width: ${zapper_psize}px; height: ${zapper_psize}px' 
 data-src='${san(
   zapper_img
 )}' src='${zapper_thumb}' onerror="javascript:replaceImgSrc(this)"
class="profile ${zapper_img ? "" : "d-none"}"></a>
</div>
<div class='col'><a class="nostr-profile-link align-middle" href='${zapper_href}' data-pubkey='${zapper_pubkey}'>${san(
      zapper_name
    )}</a>
</div>

<div class='col-auto ps-5 pe-5'> <i class="bi bi-chevron-right"></i> <b>${
      z.bolt11.msats / 1000
    } ${
      z.bolt11.msats >= 1500 ? "sats" : "sat"
    }</b> <i class="bi bi-chevron-right"></i>
</div>
...
<img style='width: ${target_psize}px; height: ${target_psize}px' 
 data-src='${san(
   target_img
 )}' src='${target_thumb}' onerror="javascript:replaceImgSrc(this)"
class="profile ${target_img ? "" : "d-none"}"></a>
</div>
<div class='col'><a class="nostr-profile-link me-1"  href='${target_href}' data-pubkey='${target_pubkey}'>${san(
      target_name
    )}</a>
</div>
...
<img style='width: ${provider_psize}px; height: ${provider_psize}px' 
 data-src='${san(
   provider_img
 )}' src='${provider_thumb}' onerror="javascript:replaceImgSrc(this)"
class="profile ${provider_img ? "" : "d-none"}"></a>
<a class="nostr-profile-link me-1" href='${provider_href}' data-pubkey='${provider_pubkey}'>${san(
      provider_name
    )}</a>
</nobr>
</div>
...
`;
  ...
  function searchNostr(req) {
    ...
    let eo = "";
    if (sort.endsWith("-day")) eo = "period_1d";
    else if (sort.endsWith("-week")) eo = "period_7d";
    else if (sort.endsWith("-month")) eo = "period_30d";

    let object_type = type;
    if (type == "profiles") object_type = "people";

    const url =
      NOSTR_API +
      "method=search&count=10&q=" +
      eq +
      (ep ? "&p=" + ep : "") +
      (es ? "&sort=" + es : "") +
      (eo ? "&options=" + eo : "") +
    ...
    $.ajax({
      ...
      .done((r) => {
        ...
        if (r.people && r.people.length) {
          ...
          for (const p of r.people) html += formatPerson({ p });

          if (r.people_count - r.people.length > 0) {
            const url = pageUrl(0, "profiles");
            html += `
<div class='mb-5'><a href='${url}' class='nostr-people-link'>And ${
              r.people_count - r.people.length
            } more profiles &rarr;</a></div>
`;
          }
        }
...
`;
          ...
          for (const u of r.serp) {
            ...
            if (u.root) html += formatEvent({ e: u.root });
...
`;
              ...
              html += `
<li class="page-item"><a class="page-link" data-page="${page}" href="${pageUrl(
                page
              )}">${label}</a></li>
`;
          }

          if (r.page > 0) formatPage(0, "First");
          if (r.page > 0) formatPage(r.page - 1, "Previous");

          let from = Math.max(0, r.page - 3);
          let till = Math.min(r.page_count, r.page + 4);
          for (let i = from; i < till; i++) formatPage(i, i + 1);

          if (r.page < r.page_count - 1) formatPage(r.page + 1, "Next");
          if (r.page < r.page_count - 1) formatPage(r.page_count - 1, "Last");
...
<` + `/script>`;
  ...
  function setQuery(q) {
    ...
    if (q) $("#advanced-bar").removeClass("d-none");
    else $("#advanced-bar").addClass("d-none");
...
<small class='text-muted ms-2'>${
            nip05 ? '<i class="bi bi-check-circle"></i>' : ""
          } ${nip05}</small>
</div>
...
`;
        ...
        let title = "";
        if (is_followers) title = "Followers " + r.followed_count;
        else title = "Following " + r.following_count;
  ...
  function getRelayIndex(r) {
    for (const i in relays) if (relays[i] == r) return i;
    return 0;
  }
  ...
  async function sendNostrMessage(tmpl, pref_relays) {
    ...
    for (const t of event[1].tags) {
      if (t.length > 2) {
        ...
        if (r) {
          for (const i in relays) if (relays[i] == r) rs.unshift(i);
        }
      }
    }
  ...
  function verifyNostrSignature(event) {
    return window.nobleSecp256k1.schnorr.verify(
      event.sig,
      event.id,
      event.pubkey
    );
  ...
  function closeSocket(relay) {
    if (relay in sockets) sockets[relay].drop();
  }
    ...
    return new Promise(function (ok, err) {
      ...
      if (!was_opened) {
        ...
        socket.drop = function () {
          for (const sub_id in socket.tos) clearTimeout(socket.tos[sub_id]);

          for (const sub_id in socket.err) socket.err[sub_id]();
        ...
        socket.done = async function (sub_id) {
          ...
          for (const v of socket.events) {
            if (v.s != sub_id) events.push(v);
            else sub_events.push(v.e);
          }
          ...
          for (const e of sub_events) {
            if (!(await validateNostrEvent(e)) || !verifyNostrSignature(e)) {
        ...
        socket.onopen = function () {
          ...
          for (const req of socket.queue) socket.send(JSON.stringify(req));
  ...
  async function getEventJson(id) {
    ...
    if (events) return JSON.stringify(events[0], null, 2);
    return "";
  }
  ...
  async function getLatestNostrEvent(kind, pubkey) {
    ...
    // nostr-band
    if ("10000" in relays) rs.push("10000");
  ...
  async function sendNostrReply(eid, root, relay, root_relay) {
    ...
    let msg = {
      kind: 1,
      content: text,
      tags: [["e", eid, relays[relay], "reply_to"]],
    };

    if (root != eid) {
      msg.tags.push(["e", root, relays[root_relay], "root"]);
    }

    msg = await sendNostrMessage(msg);
    if (msg)
      toastOk("Message sent", "Your post was submitted to Nostr network");
    else toastError("Failed to send to Nostr network");
  ...
  async function sendNostrUrlReaction(url, str) {
    ...
    msg = await sendNostrMessage(msg);
    if (!msg) toastError("Failed to send to Nostr network");
    else
      toastOk(
        "Thank you!",
        "Your reaction is now stored on the Nostr network. It will help us rank search result better, and help other people choose the best content."
      );
  }
  ...
  async function updateNostrContactList() {
    if (!window.nostr || !login_pubkey) return;
  ...
  function getContactRelays() {
    ...
    try {
      ...
      for (const r in rs) {
        if (rs[r].write) contact_relays.push(r);
      }
    } catch (e) {
  ...
  async function editPubkeyList(list, adds, dels, relay) {
    ...
    for (target of adds) {
      ...
      if (index >= 0) {
        ...
        const tag = ["p", target];
        if (relay && relay in relays) tag.push(relays[relay]);
        list.tags.push(tag);
      }
    }
  ...
  async function ensureContactList() {
    if (!latest_contact_list && login_pubkey && window.nostr)
      latest_contact_list = await getLatestNostrEvent(
        KIND_CONTACT_LIST,
        login_pubkey
      );
  }
  ...
  async function ensureFollowing(target, relay, unfollow, e) {
    ...
    // edit the list
    latest_contact_list = await editPubkeyList(
      latest_contact_list,
      adds,
      dels,
      relay
  ...
  function updateLatestList(list) {
    ...
    for (let t of list.tags) {
      if (t.length > 1 && t[0] == "p" && t[1].length == 64) list.size++;
    }
  ...
  async function ensureListed(target, relay, list_id, unlist) {
    ...
    // load lists
    if (!latest_lists) latest_lists = await getLatestLists(login_pubkey);
  ...
  async function updateLists() {
    $(".list-button").each(function (i, el) {
      ...
      if (latest_lists) {
        ...
        for (const list of latest_lists) {
          html += `
<li><button class="dropdown-item list-unlist-button" data-list='${
            list.id
          }' data-listed='${list.id in pubkey_lists}'>${
            pubkey_lists[list.id] ? '<i class="bi bi-check"></i>' : ""
          } ${list.name} <b>${list.size}</b></button></li>
`;
        }
  ...
  async function updateLabels(new_event) {
    let labels = [];
    if (latest_labels) labels.push(...latest_labels);
    ...
    $(".label-button").each(function (i, el) {
      ...
      if (labels && labels.length) {
        ...
        for (const label in unique_labels) {
          html += `
<li><button class="dropdown-item label-unlabel-button" data-label='${label}' data-labelled='${
            label in event_labels
          }'>${
            label in event_labels ? '<i class="bi bi-check"></i>' : ""
          } ${label}</button></li>
`;
        }
        html += `<li><hr class="dropdown-divider"></li>`;
  ...
  function formatEventMenu(eid, active_label) {
    ...
    if (active_label) {
      label = `
<h2><a class="btn btn-lg btn-outline-secondary open-event-overview" href="/${getNoteId(
        eid
      )}/overview"><i class="bi bi-list"></i> Overview</a> &rarr; 
${active_label}
`;
    }
      ...
      .done((r) => {
        ...
        for (const u of r.serp) {
          ...
          if (u.reply_to) {
            if (u.root)
              html += `
<div class='text-muted'><small>In a thread by @${getAuthorName(
                u.root
              )}</smalL></div>
`;
            html += formatEvent({
          ...
          if (!u.reply_to && u.root)
            html += `
<div class='text-muted'><small>Replying to @${getAuthorName(
              u.root
            )}</smalL></div>
`;
...
`;
          ...
          html += formatEvent({
            ...
            root,
            show_post: true,
            options: "thread_root,no_offset,main",
          });
          ...
          if (sub_page) {
            ...
            for (const c of u.children) {
              ...
              let options = "";
              if (c.reply_to_id == u.id) options = "no_offset";
  ...
  function formatProfileMenu(pubkey, active_label) {
    ...
    if (active_label) {
      label = `
<h2><a class="btn btn-lg btn-outline-secondary open-profile-overview" href="/${getNpub(
        pubkey
      )}/overview"><i class="bi bi-list"></i> Overview</a> &rarr; 
${active_label}
`;
      ...
      .done((r) => {
        ...
        if (r.people.length) {
          ...
          for (const p of r.people) {
            ...
            if (p.nip05 && p.nip05_verified) t += p.nip05 + ", ";
...
*/
  ...
  function showProfileEdits(pubkey) {
    ...
    const url =
      NOSTR_API + "method=profile_edits&pubkey=" + encodeURIComponent(pubkey);
...
`;
  ...
  function setRobots(index) {
    ...
    const c =
      (index ? "index" : "noindex") +
      ", follow, noimageindex, max-snippet:-1, max-image-preview:none, max-video-preview:-1, nositelinkssearchbox";
  ...
  function updateParamsState() {
      ...
      if (
        ...
        try {
          if (date) {
            ...
            if (valid_date != date) throw "Bad date";
          } else {
    ...
    if (
      params.advanced &&
      (params.advanced == "sb" || params.advanced == "rss")
    ) {
  ...
  function getBranchAttr(e, a) {
    while (e) {
      ...
      if (v) return v;
      e = e.parent();
      if (e.prop("tagName") == "HTML" || !e.prop("tagName")) break;
    }
    return "";
  }
  ...
  function attachSerpEventHandlers(sel) {
    $(sel)
      ...
      .on("click", function (e) {
        ...
        if (!window.nostr) {
          hint =
            "Please install <a href='https://getalby.com' target='_blank'>Alby</a> or some other browser extension for key storage.";
    ...
    $(sel)
      ...
      .on("click", (e) => {
        ...
        let str = "";
        for (const id of ids) str += relays[id] + "<br>";
...
`;
      ...
      if (u.threads_count > u.threads.length) {
        ...
        console.log(u.url);
        html += `
<div class='ms-5 mb-3'><small><a href='${more_url}' class='more-trending' data-url='${
          u.url
        }'>And ${
          u.threads_count - u.threads.length
        } more threads &rarr;</a></small></div>
`;
      }
    }
...
`;
      ...
      if (h.threads_count > h.threads.length) {
        html += `
<div class='ms-5 mb-3'><small><a href='${more_url}' class='more-trending' data-hashtag='${
          h.hashtag
        }'>And ${
          h.threads_count - h.threads.length
        } more threads &rarr;</a></small></div>
`;
      }
    }
...
`;
    ...
    if (dn < new Date().getTime()) next = formatDate(dn);
...
href='index.html?viewParam=trending&viewValue=${type}&viewDate=${dt}'>${
  ...
  function formatTrendingProfiles(r, date) {
    ...
    let html = "";
    if (history) html += formatTrendingHistoryHeader("profiles", date);
    ...
    $(cont + " .input-group.date")
      ...
      .on("changeDate", function (e) {
        ...
        const d = new Date(
          e.date.getTime() - e.date.getTimezoneOffset() * 60 * 1000
        );
    ...
    if (history) {
      $("#results a.previous, #results a.next, #results a.dates").on(
        "click",
        (e) => {
...
*/
  ...
  function formatTrendingPosts(r, type, date) {
    ...
    let html = "";
    if (history) html += formatTrendingHistoryHeader(type, date);
    ...
    for (const p of r[type]) {
      ...
      if (p.threads_count > p.threads.length) {
        html += `
<div class='ms-5 mb-3'><small><a href='${more_url}' class='more-trending' data-id='${
          p.post.id
        }'>And ${
          p.threads_count - p.threads.length
        } more replies &rarr;</a></small></div>
`;
      }
    }
...
`;
  ...
  function showTrending(type, date) {
    ...
    return $.ajax({
      url:
        NOSTR_API + "method=trending&type=" + tp + (date ? "&date=" + dp : ""),
    })
      .fail((x, r, e) => {
        $("#search-spinner").addClass("d-none");
        toastError("Request failed: " + e);
      ...
      .done((r) => {
        ...
        if (type == "urls") formatTrendingUrls(r);
        else if (type == "hashtags") formatTrendingHashtags(r);
        else if (type == "profiles") formatTrendingProfiles(r, date);
        else if (
          type == "posts" ||
          type == "images" ||
          type == "videos" ||
        ...
        else if (type == "zapped_posts") formatTrendingZappedPosts(r);
        ...
        if (date) {
          ...
          const triggerEl = document.querySelector(
            '#trending a.nav-link[data-bs-target="#trending-' + type + '"]'
          );
  ...
  async function getZaps(pk_id, type) {
    ...
    $.ajax({
      url:
        NOSTR_API + "method=search&type=zaps&c=10&q=" + encodeURIComponent(q),
    })
      .fail((x, r, e) => {
        $("#search-spinner").addClass("d-none");
        toastError("Request failed: " + e);
      ...
      .done(async function (rep) {
        ...
        let label = "";
        if (type == "to") label = "Latest zaps received";
        else if (type == "via") label = "Latest zaps processed";
        else if (type == "by") label = "Latest zaps sent";
        else if (type == "for") label = "Latest zaps received";
        ...
        for (const z of rep.serp) html += formatZap(z, type);
...
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Mentioned events: <b>${
          r.pub_note_ref_event_count || 0
        }</b></div>
<span class='text-muted'>Number of events mentioned by posts of this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Mentioned profiles: <b>${
          r.pub_note_ref_pubkey_count || 0
        }</b></div>
<span class='text-muted'>Number of profiles mentioned by posts of this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Reposted events: <b>${
          r.pub_repost_ref_event_count || 0
        }</b></div>
<span class='text-muted'>Number of events reposted by this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Reposted profiles: <b>${
          r.pub_repost_ref_pubkey_count || 0
        }</b></div>
<span class='text-muted'>Number of profiles reposted by this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Liked events: <b>${
          r.pub_reaction_ref_event_count || 0
        }</b></div>
<span class='text-muted'>Number of events liked by this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Liked profiles: <b>${
          r.pub_reaction_ref_pubkey_count || 0
        }</b></div>
<span class='text-muted'>Number of profiles liked by this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Reported events: <b>${
          r.pub_report_ref_event_count || 0
        }</b></div>
<span class='text-muted'>Number of events reported by this profile</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Reported profiles: <b>${
          r.pub_report_ref_pubkey_count || 0
        }</b></div>
...
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Reposting profiles: <b>${
          r.repost_pubkey_count || 0
        }</b></div>
...
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Reporting profiles: <b>${
          r.report_pubkey_count || 0
        }</b></div>
...
`;
        if (r.zaps_received) {
          html += `
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Number of zappers: <b>${
            r.zaps_received.zapper_count
          }</b></div>
<span class='text-muted'>Number of profiles that zapped this profile.</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Total amount of zaps: <b>${
            r.zaps_received.msats / 1000
          } sats</b></div>
<span class='text-muted'>Total amount of zaps received by this profile.</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Min amount of zaps: <b>${
            r.zaps_received.min_msats / 1000
          } sats</b></div>
<span class='text-muted'>Minimal amount of zaps received by this profile.</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Max amount of zaps: <b>${
            r.zaps_received.max_msats / 1000
          } sats</b></div>
<span class='text-muted'>Maximal amount of zaps received by this profile.</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Average amount of zaps: <b>${
            r.zaps_received.avg_msats / 1000
          } sats</b></div>
<span class='text-muted'>Average amount of zaps received by this profile.</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Median amount of zaps: <b>${
            r.zaps_received.median_msats / 1000
          } sats</b></div>
<span class='text-muted'>Median amount of zaps received by this profile.</span>
</div>
<div class="col-12 border-bottom mb-2 position-relative">
<div class='stats'>Number of providers: <b>${
            r.zaps_received.provider_count
          }</b></div>
...
`;
  ...
  async function getZapsOld(pk, type) {
    ...
    if (type == "to") req["#p"] = [pk];
    else if (type == "via") req["authors"] = [pk];
    ...
    for (const z of zaps) {
      ...
      if (z.target_event_id) event_ids.add(z.target_event_id);
    }
    ...
    const profile_map = {};
    for (const p of profiles) profile_map[p.pubkey] = p;
    ...
    //</h2>
    //`;
    for (const z of zaps) html += formatZap(z, type);
    ...
    $.ajax({
      ...
      .done(async function (rep) {
        ...
        if (q.startsWith("npub1") || q.startsWith("note1")) {
          ...
          note = r.type == "note";
          npub = r.type == "npub";
          if (note || npub) hex = r.data;
        }

        let html = "";
        for (const r of rep.relays) {
          if (!scanning_relays) break;
          ...
          try {
            if (note) {
              ...
              for (const n of notes) {
                const nevent = tools.nip19.neventEncode({
                  id: n.id,
                  relays: [r.u],
                });
                html += `
...
*/
    ...
    //	    q += (q ? " " : "") + except;
    if (q_by) q += (q ? " " : "") + "by:" + q_by;
    if (q_following) q += (q ? " " : "") + "following:" + q_following;
    if (q_lang) q += (q ? " " : "") + "lang:" + q_lang;
    if (q_lna) q += (q ? " " : "") + "lna:" + q_lna;
    if (q_nip05) q += (q ? " " : "") + "nip05:" + q_nip05;
    if (q && q_spam) q += " -filter:spam";
  ...
  function selectClient(client) {
    ...
    const content = $(
      '#select-client-dropdown .dropdown-item[data-client="' + client + '"]'
    ).html();
    if (!content) return;
  ...
  $("#client-open").on("click", (e) => {
    ...
    if (type == "profile") {
      ...
      if (client == "damus") url = `damus:${npub}`;
      else if (client == "amethyst") url = `nostr:${nprofile}`;
      else if (client == "nostrgram")
        url = `https://nostrgram.co/#profile:allMedia:${target}`;
      else if (client == "snort") url = `https://snort.social/p/${npub}`;
      else if (client == "iris") url = `https://iris.to/#/profile/${npub}`;
      else if (client == "astral") url = `https://astral.ninja/${npub}`;
      else if (client == "coracle") url = `https://coracle.social/${nprofile}`;
      else if (client == "guru") url = `https://www.nostr.guru/p/${target}`;
      else if (client == "satellite") url = `https://satellite.earth/@${npub}`;
      else if (client == "primal") url = `https://primal.net/profile/${npub}`;
      //	    else if (client == "plebstr")
      //	        url = `nostr:${npub}`;
      else if (client == "other-nprofile") url = `nostr:${nprofile}`;
      else url = `nostr:${npub}`;
    } else {
      ...
      if (client == "damus") url = `damus:${note}`;
      else if (client == "nostrgram")
        url = `https://nostrgram.co/#thread:${target}:${target}`;
      else if (client == "snort") url = `https://snort.social/e/${note}`;
      else if (client == "iris") url = `https://iris.to/#/post/${note}`;
      else if (client == "astral") url = `https://astral.ninja/${note}`;
      else if (client == "coracle") url = `https://coracle.social/${nevent}`;
      else if (client == "guru") url = `https://www.nostr.guru/e/${target}`;
      else if (client == "satellite")
        url = `https://satellite.earth/thread/${note}`;
      else if (client == "primal") url = `https://primal.net/thread/${note}`;
      else if (client == "other-nprofile") url = `nostr:${nevent}`;
      //	    else if (client == "plebstr")
      //		url = `nostr:${note}`;
    ...
    if (remember) localSet("chosen-client", client);
    else selectClient(localGet("chosen-client"));
  ...
  $("#trending a.nav-link").on("click", (e) => {
    ...
    const url = new URL(window.location);
    if (type != "profiles") url.searchParams.set("trending", type);
    else url.searchParams.delete("trending");
    pushUrl(url);
  ...
  $("#sort-buttons input[type=radio]").on("change", (e) => {
    ...
    if (sort.startsWith("popular")) {
      $("#sort-popular-group button.dropdown-toggle").removeClass(
        "btn-outline-secondary"
      );
      $("#sort-popular-group button.dropdown-toggle").addClass("btn-secondary");
      $("#sort-personalized-group button.dropdown-toggle").addClass(
        "btn-outline-secondary"
      );
      $("#sort-personalized-group button.dropdown-toggle").removeClass(
        "btn-secondary"
      );
      localSet("sort-popular", sort);
    } else if (sort.startsWith("personalized")) {
      $("#sort-popular-group button.dropdown-toggle").addClass(
        "btn-outline-secondary"
      );
      $("#sort-popular-group button.dropdown-toggle").removeClass(
        "btn-secondary"
  ...
  function setSort(e) {
    ...
    if (sort.startsWith("personalized")) localSet("sort-personalized", sort);
    else if (sort.startsWith("popular")) localSet("sort-popular", sort);
...
<img style='width: ${psize}px; height: ${psize}px' 
 ...
 class="profile ${
   img ? "" : "d-none"
 }" onerror="javascript:replaceImgSrc(this)"> ${name}
`;
    if (about && p.about) {
      let a = p.about;
...
`;
    ...
    $(sel)
      .find(".modal-title")
      .html(unfollow ? "Remove from list" : "Add to list");
    $(sel)
      .find("#confirm-list-update-button")
      .html(unfollow ? "Remove" : "Add");
    $(sel).attr("data-unlist", unfollow ? "true" : "");

    $(sel)
      .find(".modal-body .btn-close")
      .on("click", (e) => {
  ...
  function getTag(e, tag) {
    for (const t of e?.tags) {
      if (t.length >= 2 && t[0] == tag) return t[1];
    }
    return "";
  }
  ...
  async function getLatestLists(pubkey) {
    ...
    for (const e of events) {
      ...
      for (const t of e?.tags) {
        ...
        if (t.length > 1 && t[0] == "name") name = t[1];
        if (t.length > 1 && t[0] == "description") desc = t[1];
        if (t.length > 1 && t[0] == "p" && t[1].length == 64) size++;
      }
...
`;
    ...
    for (const l of latest_lists) {
      html += `
  <option value="${l.id}" ${
        !new_list_pubkey && last_list == l.id ? "selected" : ""
      }>${l.name}</option>
`;
    }
    if (!unlist)
      html += `<option value='' ${
        new_list_pubkey ? "selected" : ""
      }>+ New List</option>`;
    html += `
</select>
<div class='list-name ${
      !new_list_pubkey && (unlist || latest_lists) ? "d-none" : ""
    }'>
...
`;
    ...
    $("#list-update-modal .list-info select").on("change", function (e) {
      ...
      else $("#list-update-modal .list-info .list-name").addClass("d-none");
    });
...
`;
    for (p of serp) {
      if (new_list_pubkey && p.pubkey != new_list_pubkey) continue;
...
`;
    ...
    $(sel)
      .find(".modal-title")
      .html(unlist ? "Remove from list" : "Add to list");
    $(sel)
      .find("#confirm-list-update-button")
      .html(unlist ? "Remove" : "Add");
    ...
    if (new_list_pubkey) $(sel).find(".list-name input").focus();
  }
  ...
  $("#confirm-list-update-button").on("click", async function (e) {
    ...
    $("#list-update-modal .profiles .card").each((i, e) => {
      ...
      if (off != "true") (unlist ? dels : adds).push(pk);
    });
    ...
    if (lists) {
      ...
      if (!list_id) {
        const list_name = $(
          "#list-update-modal .list-info .list-name input"
        ).val();
        if (!list_name) {
  ...
  async function showUser() {
    ...
    $("#user .posts").attr(
      "href",
      "/?type=posts&q=" + encodeURIComponent(posts_q)
    );
    ...
    $("#user .following").attr(
      "href",
      "/?type=profiles&q=" + encodeURIComponent(following_q)
    );
    ...
    $("#user .feed").attr(
      "href",
      "/?type=posts&q=" + encodeURIComponent(feed_q)
    );
  ...
  $("#get-extension-pubkey-button").on("click", async () => {
    try {
      ...
      if (pubkey) $("#scope-pubkey").val(pubkey);
      else throw "Empty pubkey";
    } catch (e) {
  ...
  $("#scope-modal").on("hidden.bs.modal", (e) => {
    if (localGet("scope-pubkey")) $("#scope-personal").trigger("click");
    else $("#scope-global").trigger("click");
  });
  ...
  $("#login a, #login-ext").on("click", async function (e) {
    e.preventDefault();
    ...
    try {
      ...
      if (window.nostr) toastError("Failed to login with browser extension");
